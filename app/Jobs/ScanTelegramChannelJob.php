...
class ScanTelegramChannelJob implements ShouldQueue
{
    ...
    public function middleware(): array { return [(new WithoutOverlapping("telegram-scan-channel:{$this->channelId}"))->releaseAfter(30)->expireAfter(240)]; }
    ...
    public function handle(TelegramChannelScanner $scanner): void
    {
        ...
        if ($channel?->active) { $scanner->scan($channel, $this->limit, $this->trigger); }
    }
}

...
use App\Models\{Keyword,Lead,TelegramChannel,TelegramPost};
...
class TelegramLeadMatcherFeatureTest extends TestCase {
 ...
 public function test_channel_post_keyword_creates_one_telegram_lead_and_notification_job(): void {
  Queue::fake(); config(['services.telegram.notify_enabled'=>true]);
  ...
  $this->assertSame(1,$stats['created']); $lead=Lead::query()->firstOrFail();
  $this->assertSame('telegram',$lead->platform); $this->assertSame($post->id,$lead->source_entity_id); $this->assertSame($channel->id,$lead->channel_or_group_id); $this->assertSame("telegram:p:{$post->id}:k:{$keyword->id}",$lead->dedupe_key);
